...
本地测试 56 项通过，编译、部署预检、git diff --check 通过。提交 da02266 的 [GitHub Actions](https://github.com/zjmzvknam-dotcom/toutiao-ai/actions/runs/35356137478) 也为 success。线上已看到十种人格、新配图开关默认关闭和 AI 生图配置入口。部署更新期间旧进程短暂出现新类型导入失败，依赖安装后的进程重启已恢复，新会话正常加载页面。

线上新会话检查同时发现：写作密钥未配置到 Secrets，旧浏览器 Session 的密钥在重启后消失。因此不能宣称无需配置即可在任意手机生成。用户已授权配置 DEEPSEEK_API_KEY，但安全浏览器接口未能读取并传递本机凭据，未打印密钥、未提交空配置，设置页面已留待用户填写。
