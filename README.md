...
热点雷达现在默认接入两个无需模型 Token 的公开源：Google Trends 香港中文 RSS（用于中文区域搜索趋势）和 douyinhuo.cn 公共聚合接口（包含抖音、今日头条、微博、百度、知乎、哔哩哔哩等榜单）。页面按 15 分钟缓存，展示来源与刷新时间；某个源失败会保留其他源，并在没有实时结果时读取每日 GitHub Actions 快照。公开榜单属于选题信号，不是事实核验结果，也不保证平台审核或流量。
